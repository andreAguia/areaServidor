...
$idLotacao = $pessoal->get_idLotacao($idServidorLogado);

# Pega os parâmetros dos relatórios
$anoBase = (int) post('anoBase', date('Y'));
$mesBase = (int) post('mes', date('m'));
$idServidorPesquisado = (int) post('idServidorPesquisado', $idServidorLogado);
...
if (Verifica::acesso($idUsuario, [1, 3, 9, 10, 11])) {
    ...
    # Monta o select da combo servidor
    $result3 = $pessoal->select("SELECT DISTINCT tbservidor.idServidor,
                                        tbpessoa.nome
    ...
    $relatorio->set_formCampos(array(
        array('nome' => 'idServidorPesquisado',
            'label' => 'Servidor:',
            'tipo' => 'combo',
            'size' => 3,
            'title' => 'Ano',
