...
mysqldump -h localhost -u root -pDSvuEtwz6h9HfLCF grh > C:/_backup/%date:~6,10%.%date:~3,2%.%date:~0,2%.%ftime%.%time:~3,2%.grh.sql
mysqldump -h localhost -u root -pDSvuEtwz6h9HfLCF areaServidor > C:/_backup/%date:~6,10%.%date:~3,2%.%date:~0,2%.%ftime%.%time:~3,2%.areaServidor.sql

REM compacta arquivos
SET PATH=%PATH%;C:\Program Files\7-Zip
7z a C:\_backup\%date:~6,10%.%date:~3,2%.%date:~0,2%.%ftime%.%time:~3,2%.7z C:\_backup\*.sql
...
IF NOT EXIST C:\Users\gti\"Google Drive"\%date:~6,10%\ (
	md C:\Users\gti\"Google Drive"\%date:~6,10%\
)

REM Verifica e cria a pasta do mes no google drive
IF NOT EXIST C:\Users\gti\"Google Drive"\%date:~6,10%\%date:~3,2%\ (
	md C:\Users\gti\"Google Drive"\%date:~6,10%\%date:~3,2%\
)

REM Copia os arquivos do dia para o google drive
copy C:\_backup\%date:~6,10%.%date:~3,2%.%date:~0,2%*.* C:\Users\gti\"Google Drive"\%date:~6,10%\%date:~3,2%\
